import React, { useState } from 'react'
import { LuChevronDown, LuEclipse } from 'react-icons/lu';

const SelectDropDownFromFetch = ({options, value, onChange, placeholder}) => {
    const [isOpen, setIsOpen] = useState(false)

    const handleSelect = (option) => {
        onChange(option);
        setIsOpen(false)
    };

  return (
    <div className='relative w-full'>
        {/* Dropdown button */}
        <button
        onClick={() => setIsOpen(!isOpen)}
        className='w-full text-sm text-black outline-none bg-white border border-slate-100 px-2.5 py-3 rounded-md mt-2 flex items-center justify-between'
        >
            {value ? options.find((opt) => opt._id === value)?.name : placeholder}
            <span className='ml-2'>{isOpen ? <LuChevronDown className='rotate-180' />: <LuChevronDown />}</span>
        </button>

        {/* Dropdown menu */}
        { isOpen && (
            <div className='absolute w-full bg-white border border-slate-100 rounded-md mt-1 shadow-md z-10'>
                {options.map((option) => (
                    <div
                    key={option._id}
                    onClick={() => handleSelect(option._id)}
                    className='px-3 py-2 text-sm cursor-pointer hover:bg-gray-100'
                    >
                        {option.name}
                    </div>
                ))}
            </div>
        )}
    </div>
  )
}

export default SelectDropDownFromFetch