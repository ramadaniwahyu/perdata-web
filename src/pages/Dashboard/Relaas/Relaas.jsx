...
const Relaas = () => {
    ...
    const [inputTglSidang, setInputTglSidang] = useState(false)
    ...
    const getRelaas = async () => {
        ...
        try {
            ...
            setAllRelaas(response.data.panggilan.length > 0 ? response.data.panggilan : [{ emptyRelaas }])
        } catch (error) {
            toast.error(error.response.data.msg)
            console.error(error.response.data.desc)
        } finally {
        ...
        return (
            <>
                <div className=' flex items-center justify-between gap-4'>
                    <div tooltip="Ubah" className={`flex items-center text-[11px] hover:text-blue-500 text-blue-300 font-medium px-4 py-0.5 mt-4 cursor-pointer`} onClick={() => openEditRelaas(rowData)}>
                        <LuFilePen className='text-base' size={25} />&nbsp;Ubah

                    </div>
                    <div className={`flex items-center text-[11px] hover:text-yellow-500 text-yellow-300 font-medium px-4 py-0.5 mt-4 cursor-pointer`} >
                        <LuFileBadge className='text-base' size={25} />&nbsp;Detail
                    </div>
                    <div className={`flex items-center text-[11px] hover:text-red-500 text-red-300 font-medium px-4 py-0.5 mt-4 cursor-pointer`} onClick={() => openDeleteRelaas(rowData)}>
                        <LuTrash2 className='text-base' size={25} />&nbsp;Hapus
                    </div>
                </div>
            </>
        )
        ...
        <DashboardLayout activeMenu="Relaas" breadcrumb={breadcrumb}>
            ...
            <div className='flex flex-col lg:flex-row lg:items-center justify-between'>
                <div className='col-span-3'>
                    ...
                    <p className='text-xs md:text-[13px] text-gray-400 mt-1.5'>
                        Halaman untuk mengatur Relaas Panggilan / Pemberitahuan melalui Surat Tercatat.
                    </p>
                </div>
            ...
            <Modal isOpen={isOpenNew} onClose={closeNew}>
                <div className='flex items-center justify-between'>
                    <h2 className="text-xl font-medium mb-4">{onEdit ? "Update Relaas" : "Tambah Relaas"}</h2>
                </div>
                <div className='mt-2'>
                    <div className='mt-1'>
                    ...
                    <div className='mt-1'>
                        ...
                        <input
                            ...
                            onChange={({ target }) => handleValueChange("tglKirim", target.value)
                            }
                            type='date'
                        />
                    </div>
                    <div className='mt-1'>
                        ...
                        <input
                            ...
                            onChange={({ target }) => handleValueChange("nomorPerkara", target.value)
                            }
                            type='text'
                        />
                    </div>
                    <div className='mt-1'>
                        ...
                        <input
                            ...
                            className='form-input'
                            value={relaas.pihak}
                            onChange={({ target }) => handleValueChange("pihak", target.value)
                            }
                            type='text'
                        />
                    </div>
                    <div className='mt-1'>
                        ...
                        <input
                            ...
                            className='form-input'
                            value={relaas.alamat}
                            onChange={({ target }) => handleValueChange("alamat", target.value)
                            }
                            type='text'
                        />
                    </div>
                    <div className='mt-1'>
                        ...
                        <input
                            ...
                            onChange={({ target }) => handleValueChange("tglSidang", target.value)}
                            type='date' />
                        <p className='text-xs font-light italic text-gray-500'>Silahkan isi bila merupakan Panggilan Sidang</p>
                    </div>
                    <div className='mt-1'>
                        <label className='text-xs font-medium text-slate-600'>
                            Due Date
                    ...
                    <div className='flex items-center justify-end gap-1.5 p-3'>
                        <button onClick={closeNew} className="mt-4 bg-red-300 px-4 py-2 rounded" disabled={loading}>
                            Kembali
                        </button>
            ...
            <Modal isOpen={isOpenDelete} onClose={() => setIsOpenDelete(false)}>
                ...
                <div className='mt-4'>
                    ...
                    <div className='flex items-center justify-end gap-1.5 p-3'>
                        <button onClick={() => setIsOpenDelete(false)} className="mt-4 bg-green-300 px-4 py-2 rounded">
                            Kembali
                        </button><button onClick={() => deleteRelaas()} className="mt-4 bg-red-300 px-4 py-2 rounded">
